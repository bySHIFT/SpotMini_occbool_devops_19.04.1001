...
namespace internal
{
    ...
    public:
        ...
        {
            return task<typename std::result_of<F(R)>::type(Args...)>([this, &f](Args&&... args){ return f(m_fn(std::forward<Args>(args)...)); });
        }
...
#undef RUN_NEXT_JOB
    ...
    void TestTask()
    {
        task <cicd&(cicd&)> my_cicd = [](cicd& once_cicd) -> cicd& { return once_cicd; };
        ...
        std::cout << std::endl << ">> STATUS: " << (steps == cicd::status::ALL ? "DONE" : "ERROR") << std::endl << std::endl;
    }
