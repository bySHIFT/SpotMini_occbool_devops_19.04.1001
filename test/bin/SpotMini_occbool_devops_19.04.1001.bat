...
SET CI_MQ_CALC_REQUEST_HOST_CONN_PARAMS=guest:guest@192.168.1.95:5672

REM 请求用 算量 RabbitMQ 队列名称(OCCBOOL 算量用)
SET CI_MQ_CALC_REQUEST_CMD_NAME=CALC_OCCBOOL_DEVOPS_1010

REM 回复用 算量 RabbitMQ 服务相关情报(OCCBOOL 算量用)
REM   设定格式: user:password@host:port
SET CI_MQ_CALC_RESPONSE_HOST_CONN_PARAMS=guest:guest@192.168.1.95:5672

REM 回复用 算量 RabbitMQ 队列名称(OCCBOOL 算量用)
SET CI_MQ_CALC_RESPONSE_CMD_NAME=CALC_OCCBOOL_DEVOPS_2010
...
REM   设定类型: 无符号型(0: 全计算)
SET CI_MAX_COUNT_CATEGORY=99

SpotMini_occbool_devops_19.04.1001.exe
